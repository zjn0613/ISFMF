...
def ISFMF(res_dir, train_user, train_item, valid_user, test_user,
    ...
    for iteration in range(max_iter):
        ...
        history = isf_module.train(image, V, item_weight, seed)
        theta = isf_module.get_projection_layer(image)
        ...
        if (val_eval < pre_val_eval):
            isf_module.save_model(res_dir + '/CNN_weights.hdf5')
            np.savetxt(res_dir + '/U.dat', U)
            np.savetxt(res_dir + '/V.dat', V)
            np.savetxt(res_dir + '/theta.dat', theta)
        else:
...
def PMF(res_dir, train_user, train_item, valid_user, test_user,
